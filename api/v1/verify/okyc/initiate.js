...
const OKYC_INITIATE_API_URL = process.env.CGEPY_OKYC_INITIATE_URL || process.env.CGPEY_OKYC_INITIATE_URL || 'https://verify.cgpey.com/api/v1/verify/okyc/initiate';
...
function logVerificationIpContext(req, details) {
  ...
  console.warn('[OKYC_INITIATE] verification failed - requestIp=' + (requestIp || 'unknown') + ', upstreamClientIP=' + (upstreamClientIp || 'unknown') + ', requestId=' + (requestId || 'n/a'));
}
...
module.exports = async function handler(req, res) {
  ...
  const normalizedInitiateUrl = (OKYC_INITIATE_API_URL || '').replace(/\/$/, '');
  ...
  try {
    const response = await fetch(OKYC_INITIATE_API_URL, {
      method: 'POST',
