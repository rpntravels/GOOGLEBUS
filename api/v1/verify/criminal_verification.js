...
const CRIMINAL_VERIFY_API_URL = process.env.CGEPY_CRIMINAL_VERIFY_URL || process.env.CGPEY_CRIMINAL_VERIFY_URL || 'https://verify.cgpey.com/api/v1/verify/criminal_verification';
...
function logVerificationIpContext(req, details) {
  ...
  console.warn('[CRIMINAL_VERIFICATION] verification failed - requestIp=' + (requestIp || 'unknown') + ', upstreamClientIP=' + (upstreamClientIp || 'unknown') + ', requestId=' + (requestId || 'n/a'));
}
...
module.exports = async function handler(req, res) {
  ...
  const normalizedVerifyUrl = (CRIMINAL_VERIFY_API_URL || '').replace(/\/$/, '');
  ...
  try {
    const response = await fetch(CRIMINAL_VERIFY_API_URL, {
      method: 'POST',
      headers: {
